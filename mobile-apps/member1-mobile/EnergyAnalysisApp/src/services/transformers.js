...
export function normalizeWeatherData(raw = {}) {
  ...
  // Check for nested temp objects (like temperature: { current: 28 })
  const tempCandidate = w.temperature?.current ?? w.temp ?? w.temperature ?? (w.temp_c ?? w.temp_f) ?? null;
  const temp = typeof tempCandidate === 'number' ? tempCandidate : Number(tempCandidate) || null;
  // Check for nested weather condition (like weather: { description: "scattered clouds" })
  const condition = w.weather?.description ?? w.weather?.main ?? w.condition ?? w.summary ?? (w.desc ?? null);
  ...
  // Safety suggestions
  let suggestions = [];
  if (Array.isArray(raw.safetySuggestions)) {
    suggestions = raw.safetySuggestions;
  } else if (raw.safetySuggestions && typeof raw.safetySuggestions === 'object') {
    suggestions = [
      ...(raw.safetySuggestions.immediate || []),
      ...(raw.safetySuggestions.preventive || []),
      ...(raw.safetySuggestions.emergency || []),
    ];
    if (suggestions.length === 0 && raw.safetySuggestions.message) {
      suggestions = [raw.safetySuggestions.message];
    }
  } else {
    suggestions = raw.suggestions ?? raw.safety ?? [];
  }

  out.safetySuggestions = suggestions;
