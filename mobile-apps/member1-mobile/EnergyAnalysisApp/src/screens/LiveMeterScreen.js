/**
 * LiveMeterScreen.jsx  — EnergyIQ Appliance Tester (FIXED)
 *
 * Fixes applied:
 ...
 *  ✅ Download JSON/CSV after session completes
...
const POLL_INTERVAL_MS = 3000;   // REST polling interval when WS is down
const WS_RECONNECT_MS  = 6000;   // WebSocket reconnect delay (longer = less spam)
...
const rb = StyleSheet.create({
  ...
  shimmer: { position:'absolute', top:0, width:'30%', height:'100%',
...
const AnomalyBanner = ({ anomalies }) => {
  ...
  const ICONS = { LOW_VOLTAGE:'⚡', HIGH_VOLTAGE:'⚡', LOW_PF:'📉', CRITICAL_LOW_PF:'🔴',
...
const ab = StyleSheet.create({
  wrap: { backgroundColor:'#2D1515', borderRadius:10, borderLeftWidth:3,
...
const sc = StyleSheet.create({
  wrap:  { flex:1, alignItems:'center', paddingVertical:12,
           borderRightWidth:1, borderColor:C.border },
    ...
    try {
      const r = await fetch(`${API_BASE}/iot/sessions`, {
        ...
        body: JSON.stringify({
          ...
          test_duration_min:     parseInt(form.test_duration_min) || 15,
        }),
      });
      const d = await r.json();
      if (!r.ok) throw new Error(d.detail || 'Failed');
      onStart(d.session_id, form.appliance_name, parseInt(form.test_duration_min) || 15);
  ...
  return (
    <ScrollView style={s.flex} contentContainerStyle={{ padding:20, paddingBottom:40 }}
        ...
        <View style={{ flexDirection:'row', flexWrap:'wrap', gap:8, marginTop:6 }}>
          {DURATIONS.map(d => (
            <TouchableOpacity key={d}
              style={[s.chip, form.test_duration_min === d && s.chipActive]}
              onPress={() => set('test_duration_min')(d)}>
              <Text style={[s.chipTxt, form.test_duration_min === d && { color:C.accent, fontWeight:'700' }]}>
                {d} min
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <TouchableOpacity style={[s.primaryBtn, loading && { opacity:0.6 }]} onPress={start} disabled={loading}>
        {loading ? <ActivityIndicator color="#000" /> : <Text style={s.primaryTxt}>▶  Start Test</Text>}
      </TouchableOpacity>
...
// ─────────────────────────────────────────────────────────────────────────────
// SCREEN 3 — MONITOR  (major fixes here)
// ─────────────────────────────────────────────────────────────────────────────
const MonitorScreen = ({ account, deviceId, sessionId, applianceName, plannedMin, onDone, onStop }) => {
  ...
  const [polling,   setPolling]   = useState(false);   // REST polling active
  ...
  const connectWS = useCallback(() => {
    ...
    try {
      ...
      ws.onmessage = ev => {
        ...
        try {
          ...
          if (msg.type === 'live_reading') {
            ...
            // snapshot: { latest, history, events, active_session }
            ...
            // Backend pushed session_ended — use it
      ...
      ws.onclose = () => {
        ...
        startPolling();
        // Schedule reconnect (don't spam — wait 6s)
  ...
  // ── Stop session — POST then poll for completion ───────────────────────────
  const stopSession = async () => {
    setStopping(true);
    setStopMsg('Stopping session…');

    // Close WS so it stops reconnecting
    wsAlive.current = false;
    ...
    try {
      ...
      if (!r.ok && r.status !== 400) {
        ...
        // If 400 = already abandoned, go back
      ...
      // Poll /sessions/:id until status === "completed"
      let attempts = 0;
      const maxAttempts = 40;  // up to 2 minutes
      const pollForCompletion = async () => {
        if (!mountedRef.current) return;
        if (attempts++ > maxAttempts) {
          // Timeout — try to show whatever we have
          try {
...
const panel = StyleSheet.create({
  card:         { backgroundColor:C.card, borderRadius:14, padding:4, marginBottom:10,
                  borderWidth:1, borderColor:C.border, overflow:'hidden' },
  sectionTitle: { color:C.textMuted, fontSize:11, textTransform:'uppercase', letterSpacing:0.7,
...
const mn = StyleSheet.create({
  pill:        { flexDirection:'row', alignItems:'center', gap:5, paddingHorizontal:10,
  ...
  stoppingBox: { flexDirection:'row', alignItems:'center', justifyContent:'center',
...
const ReportScreen = ({ sessionData, onNewTest }) => {
  ...
  const health = (() => {
    try { return typeof sessionData?.health_assessment === 'string'
  ...
  const doExport = async (fmt) => {
    ...
    try {
      ...
      await Share.share({
        title:   `EnergyIQ – ${st.appliance_name} – ${fmt.toUpperCase()}`,
        message: text,
      });
  ...
  return (
    <ScrollView style={s.flex} contentContainerStyle={{ padding:20, paddingBottom:50 }}>
      ...
      <View style={rp.section}>
        ...
        <TouchableOpacity style={[rp.shareBtn, { backgroundColor:C.card, marginBottom:10 }]} onPress={shareReport}>
        ...
        <View style={{ flexDirection:'row', gap:10 }}>
          <TouchableOpacity style={rp.exportBtn} onPress={() => doExport('json')} disabled={exporting}>
            <Text style={{ color:C.accent, fontWeight:'700' }}>⬇ JSON</Text>
            <Text style={{ color:C.textMuted, fontSize:11, marginTop:2 }}>Full dataset</Text>
          </TouchableOpacity>
          <TouchableOpacity style={rp.exportBtn} onPress={() => doExport('csv')} disabled={exporting}>
            <Text style={{ color:C.green, fontWeight:'700' }}>⬇ CSV</Text>
            <Text style={{ color:C.textMuted, fontSize:11, marginTop:2 }}>ML-ready</Text>
          </TouchableOpacity>
        </View>
        ...
        {exporting && <ActivityIndicator color={C.accent} style={{ marginTop:8 }} />}
      </View>
...
const rp = StyleSheet.create({
  statsGrid:      { flexDirection:'row', flexWrap:'wrap', backgroundColor:C.card, borderRadius:14,
                    borderWidth:1, borderColor:C.border, overflow:'hidden', marginBottom:10 },
  statCell:       { width:'33.33%', alignItems:'center', paddingVertical:14,
  ...
  section:        { backgroundColor:C.card, borderRadius:14, padding:14, marginBottom:10,
  ...
  shareBtn:       { borderRadius:12, paddingVertical:13, alignItems:'center',
                    borderWidth:1, borderColor:C.border },
  exportBtn:      { flex:1, backgroundColor:C.accentDim, borderRadius:10, paddingVertical:12,
...
const s = StyleSheet.create({
  ...
  input:      { backgroundColor:C.card, color:C.textPrimary, borderRadius:10,
  ...
  primaryBtn: { marginHorizontal:20, marginVertical:16, backgroundColor:C.accent,
                borderRadius:12, paddingVertical:14, alignItems:'center' },
  primaryTxt: { color:'#000', fontWeight:'700', fontSize:15 },
  outlineBtn: { margin:20, borderWidth:1, borderColor:C.border, borderRadius:10,
                paddingVertical:12, alignItems:'center' },
  outlineTxt: { color:C.textSec, fontSize:14 },
  devCard:    { backgroundColor:C.card, borderRadius:12, padding:16, marginBottom:10,
