...
export const ElectricianEarnedScreen = () => {
    ...
    React.useEffect(() => {
        const fetchEarnings = async () => {
            if (!user?.id) return;
            try {
                const response = await fetch(`http://192.168.8.101:8003/api/earnings/${user.id}`);
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                {(earningsData?.commissionOwed || 0) > 0 && (
                    <Card style={[styles.chartCard, { backgroundColor: theme.colors.danger + '10', borderColor: theme.colors.danger + '40', borderWidth: 1 }]} glowColor={theme.colors.danger}>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            ...
                            <View style={{ flex: 1 }}>
                                <Text style={{ ...theme.typography.body, color: theme.colors.danger, fontWeight: '700' }}>Commission Owed: LKR {earningsData.commissionOwed}</Text>
                                <Text style={{ ...theme.typography.caption, color: theme.colors.textSecondary, marginTop: 4 }}>Please settle this amount to avoid account suspension.</Text>
                            </View>
                        </View>
                    </Card>
                )}
                ...
                {transactions.length > 0 ? transactions.map((tx) => (
                    ...
                    <Text style={{ ...theme.typography.body, color: theme.colors.textSecondary, textAlign: 'center', marginVertical: 20 }}>No recent transactions found.</Text>
                )}
            </ScrollView>
        </SafeAreaView>
    );
